...
export function useTerminal({ sessionId }: UseTerminalOptions) {
  ...
  // Reset createdRef when connection drops so we re-create on reconnect
  useEffect(() => {
  ...
  const attach = useCallback(
    (container: HTMLDivElement | null) => {
      ...
      // Addons (fit must be loaded before open for proper sizing)
      const fitAddon = new FitAddon();
      fitAddonRef.current = fitAddon;
      terminal.loadAddon(fitAddon);

      // Mount after browser completes layout to avoid _renderService race condition.
      // xterm's Viewport.syncScrollArea can fire before _renderService is ready
      // if terminal.open() is called during React's commit phase (before paint).
      // WebLinksAddon must be loaded AFTER open() in xterm 6.x due to DI changes.
      const openTerminal = () => {
        if (container.offsetWidth === 0 || container.offsetHeight === 0) {
          // Container not yet visible — wait for next frame
        ...
        try {
          ...
          // fit() may fail before renderer is ready
        }
        sendTerminalCreateOrAttach();
      };
      requestAnimationFrame(openTerminal);
      ...
      const unsubDestroyed = transportManager.on(
        MessageType.TERMINAL_DESTROYED,
        (msg: VPMessage) => {
          const payload = msg.payload as any;
