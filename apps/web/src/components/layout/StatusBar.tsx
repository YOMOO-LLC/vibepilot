...
const stateColor = (state: string) =>
  ...
  state === 'failed' ? 'bg-red-500' : 'bg-zinc-500';
...
export function StatusBar() {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-1 bg-zinc-900 border-t border-zinc-800 text-xs text-zinc-500" data-testid="status-bar">
      <div className="flex items-center gap-4">
        <span>{tabCount} terminal{tabCount !== 1 ? 's' : ''}</span>
